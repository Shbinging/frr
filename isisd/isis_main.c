 ...
 */
 ...
 static struct isis_circuit *FuzzingCreateIsis(void)
 {
	 ...
	 bool created;
	 struct isis_area* a = isis_area_create("1", VRF_DEFAULT_NAME);
	 struct isis_circuit *c = isis_circuit_new(ifp, "1");
	 isis_area_add_circuit(a, c);
	 struct connected *co = connected_add_by_prefix(ifp, &p, NULL);
	 isis_circuit_if_add(c, ifp);
	 c->state = C_STATE_UP;
	 c->fd = 69;
	 return c;
 }
 ...
 int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
 {
	 ...
	 circuit = FuzzingIsis;
	 uint8_t ssnpa[ETH_ALEN] = {0x00, 0x1A, 0x2B, 0x3C, 0x4D, 0x5E};
	 ...
	 stream_write(circuit->rcv_stream, data, size);
